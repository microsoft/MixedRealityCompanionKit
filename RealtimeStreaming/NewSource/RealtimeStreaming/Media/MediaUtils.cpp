	 ...
	 UINT32* height)
{
	 ...
	 IFT(mediaEncodingProfile->get_Video(&spVideoEncodingProperties));

	 IFT(spVideoEncodingProperties->get_Height(height));
	 IFT(spVideoEncodingProperties->get_Width(width));
	 ...
	 IDirect3DSurface** ppSurface)
{
	 ...
	 *ppSurface = nullptr;
	 ...
	 IFT(spTexture.As(&dxgiSurface));

	 com_ptr<IInspectable> inspectableSurface;
	 IFT(CreateDirect3D11SurfaceFromDXGISurface(dxgiSurface.get(), &inspectableSurface));

	 com_ptr<IDirect3DSurface> spSurface;
	 IFT(inspectableSurface.As(&spSurface));
    ...
    IMFMediaBuffer** ppIMFBuffer)
{
    ...
    BYTE *pMFdata = NULL;
    IFT(MFCreateMemoryBuffer(cbBuffer, &spBuffer));
