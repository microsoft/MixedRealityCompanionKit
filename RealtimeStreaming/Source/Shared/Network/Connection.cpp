...
HRESULT Connection::WaitForHeader()
{
    ...
    //check_hresult(m_spHeaderBuffer->get_MediaBuffer(&spMediaBuffer));
...
HRESULT Connection::WaitForPayload()
{
    ...
    //check_hresult(payloadBuffer.MediaBuffer(&spMediaBuffer));
    ...
    {
        check_hresult(spMediaBuffer->SetCurrentLength(m_receivedHeader.cbPayloadSize))
    }
