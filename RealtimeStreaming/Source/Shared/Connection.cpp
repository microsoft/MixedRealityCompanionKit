...
winrt::fire_and_forget Connection::RunSocketLoop()
{
	...
	try
	{
		// TODO: bit concerned that there's a race here around setting the socket to nullptr
		// as we don't take the lock on iterations of this function.
		IFT(CheckClosed());

		Buffer sizeBuffer(sizeof(UINT32));
		auto inputStream = m_streamSocket.InputStream();

		co_await inputStream.ReadAsync(sizeBuffer, sizeBuffer.Capacity(), InputStreamOptions::None);
	...
	RealtimeStreaming::Network::DataBundle const dataBundle)
{
	...
	try
	{
		...
		// Write size of data that we are sending over the wire.
		Buffer sizeBuffer(sizeof(UINT32));
		*((UINT32*)sizeBuffer.data()) = totalLength;
		sizeBuffer.Length(sizeof(UINT32));

		auto outStream = m_streamSocket.OutputStream();
		co_await outStream.WriteAsync(sizeBuffer);
...
{
	// TODO: more explicit error codes here?
